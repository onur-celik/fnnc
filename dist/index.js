...
function MA(closingPrices, period) {
...
function EMA(closingPrices, period) {
    const k = 2 / (period + 1);
...
function Uptrend(closingPrices, maPeriod) {
    /*
     * More information about this function can be
...
function WilliamsPercentR(closingPrices) {
    /*
     * More information about this function can be
    ...
    // Calculate the %R
    const percentR = (100 * (closingPrices[closingPrices.length - 1] - highestHigh)) /
        (highestHigh - lowestLow);
    return percentR;
}
function StandardDeviation(closingPrices) {
    let sum = 0;
...
function BollingerBands(closingPrices, numStdDev) {
    /*
     * More information about this function can be
...
function Volatility(closingPrices) {
    /*
     * More information about this function can be
...
function DMI(prices, periods) {
    ...
    for (let i = 0; i < prices.length; i++) {
        ...
        let tr = Math.max(prices[i].high - prices[i].low, Math.abs(prices[i].high - prices[i - 1].close), Math.abs(prices[i].low - prices[i - 1].close));
        let atrValue = 0;
        if (i < periods) {
            atrValue = tr;
        }
